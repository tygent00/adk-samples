...
async def main():
    ...
    base_time, base_tokens, base_output = await _run(academic_coordinator, question)

    accelerated_run = accelerate(_run)
    acc_time, acc_tokens, acc_output = await accelerated_run(academic_coordinator, question)
